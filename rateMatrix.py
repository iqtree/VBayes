...
def decompHKY(pden, kappa, symm=False):
    ...
    if not symm:
        ...
        D_HKY, W_HKY = np.linalg.eigh(
            np.dot(np.dot(np.diag(np.sqrt(pden)), rate_matrix_HKY), np.diag(np.sqrt(1.0 / pden))))
...
def decompGTR(pden, AG, AC, AT, GC, GT, CT, symm=False):
    ...
    if not symm:
        ...
        D_GTR, W_GTR = np.linalg.eigh(
            np.dot(np.dot(np.diag(np.sqrt(pden)), rate_matrix_GTR), np.diag(np.sqrt(1.0 / pden))))

...
def summary(path, schema, burnin=0, n_rep=10):
    ...
    for i in range(1, n_rep + 1):
        tree_dict_rep, tree_counts_rep, tree_ids_rep = tree_summary(path + '_rep_{}'.format(i) + '.trees', schema,
                                                                    burnin)
    ...
    with open(filename, 'r') as input_file:
        while True:
            ...
            if truncate and num_trees == truncate + burnin:
                break
    tree_wts = [tree_wts_dict[tree_dict[tree_name].get_topology_id()] / (num_trees - burnin) for tree_name in
                tree_names]
    if hpd < 1.0:
...
def summary_raw(file_path, truncate=None, hpd=0.95, n_rep=10):
    ...
    for i in range(1, n_rep + 1):
        tree_dict_rep, tree_names_rep, tree_wts_rep = get_tree_list_raw(file_path + '_rep_{}'.format(i),
                                                                        truncate=truncate, hpd=hpd)
...
def get_support_info(taxa, sampled_tree_dict):
    ...
    for tree in sampled_tree_dict.values():
        nodetobitMap = {node: toBitArr.from_clade(node.get_leaf_names()) for node in tree.traverse('postorder') if
                        not node.is_root()}

...
class PHY(object):
    def __init__(self, data, taxa, pden, subModel, scale=0.1, unique_site=True, use_ambiguity=True):
        ...
        if unique_site:
            L_np, site_counts_np = self.initialCLV(data, unique_site=True)
            self.L = torch.from_numpy(np.array(L_np)).float()
            self.site_counts = torch.from_numpy(np.array(site_counts_np)).float()
        else:
    ...
    def loglikelihood(self, branch, tree):
        ...
        transition_matrix = torch.matmul(torch.einsum("ij,kj->kij", (self.U, torch.exp(branch_D))), self.U_inv).clamp(
            0.0)
        scaler_list = []
